...
local function createActionButton(name, posScaleY)
    ...
    btn.InputBegan:Connect(function(input)
        if not IS_GLOBAL_LOCKED and (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) then
            local dragging = true; local dragStart = input.Position; local startPos = btn.Position
            ...
            endCon = UIS.InputEnded:Connect(function(endInput)
                if endInput.UserInputType == Enum.UserInputType.MouseButton1 or endInput.UserInputType == Enum.UserInputType.Touch then
                    ...
                    moveCon:Disconnect()
                    endCon:Disconnect()
                    tweenObj(btn, {Size = UDim2.new(0, globalBtnSize, 0, globalBtnSize)}, 0.15)
                end
            end)
        end
    end)
...
Instance.new("UIStroke", SizeMenu).Color = Color3.fromRGB(255, 255, 255); 
Instance.new("UIStroke", SizeMenu).Transparency = 0.5
...
local function createFeature(text, linkedBtn, layoutOrder, slidersConfig, subTogglesConfig)
    ...
    local function setVis(item, show)
        ...
        if show then
            ...
            tw.Completed:Connect(function() if not item.isShowing then item.frame.Visible = false end end)
        end
    end
    ...
    if subTogglesConfig then
        for i, cfg in ipairs(subTogglesConfig) do
            ...
            stContainer.Size = UDim2.new(1, 0, 0, 0); stContainer.BackgroundTransparency = 1; stContainer.ClipsDescendants = true
            stContainer.Visible = false; stContainer.LayoutOrder = 100 + i; stContainer.Parent = group
            ...
            stLb.Font = Enum.Font.GothamMedium; stLb.TextSize = 11; stLb.TextXAlignment = Enum.TextXAlignment.Left; stLb.BackgroundTransparency = 1; stLb.Parent = stContainer
            
            local stBg = Instance.new("TextButton")
            stBg.Text = ""; stBg.Size = UDim2.new(0, 34, 0, 18); stBg.Position = UDim2.new(1, -42, 0.5, -9); stBg.BackgroundColor3 = Color3.fromRGB(40, 40, 40); stBg.Parent = stContainer
            Instance.new("UICorner", stBg).CornerRadius = UDim.new(1, 0)
            
            local stKnob = Instance.new("Frame")
            stKnob.Size = UDim2.new(0, 12, 0, 12); stKnob.Position = UDim2.new(0, 3, 0.5, -6); stKnob.BackgroundColor3 = Color3.fromRGB(150, 150, 150); stKnob.Parent = stBg
            Instance.new("UICorner", stKnob).CornerRadius = UDim.new(1, 0)
            
            local stActive = false
            table.insert(visualItems, {frame = stContainer, type = "subToggle", targetHeight = 24, isShowing = false})
            ...
            stBg.MouseButton1Click:Connect(function()
                ...
                if isMainSpeedGlitchEnabled then
                    for _, item in ipairs(visualItems) do
                        ...
                        elseif item.type == "macroToggle" then setVis(item, isLegitSpeedEnabled) end
                    end
                end
            end)
        end
    ...
    switchBg.MouseButton1Click:Connect(function()
        ...
        if text == "Speed Glitch" then isMainSpeedGlitchEnabled = isActive end
        if isActive then
            ...
            if linkedBtn and not isLegitSpeedEnabled then linkedBtn.Visible = true end
            for _, item in ipairs(visualItems) do
                if item.type == "subToggle" then setVis(item, true)
                elseif item.type == "main" then setVis(item, not isLegitSpeedEnabled)
                elseif item.type == "legit" or item.type == "macroToggle" then setVis(item, isLegitSpeedEnabled) end
            end
        else
            ...
            for _, item in ipairs(visualItems) do setVis(item, false) end
        end
    end)
end
-- ==========================================
...
EditModeFrame.Size = UDim2.new(1, 0, 0, 50); EditModeFrame.BackgroundColor3 = Color3.fromRGB(0, 0, 0); EditModeFrame.BackgroundTransparency = 0.5; EditModeFrame.Parent = ContentFrame; EditModeFrame.LayoutOrder = 0; Instance.new("UICorner", EditModeFrame).CornerRadius = UDim.new(0, 8)

local EditBtn = Instance.new("TextButton")
EditBtn.Size = UDim2.new(0, 85, 0, 28); EditBtn.Position = UDim2.new(1, -100, 0.5, -14); EditBtn.Text = "COMBAT"; EditBtn.BackgroundColor3 = Color3.fromRGB(0, 0, 0); EditBtn.TextColor3 = Color3.fromRGB(255, 255, 255); EditBtn.Font = Enum.Font.GothamBold; EditBtn.TextSize = 11; EditBtn.Parent = EditModeFrame; Instance.new("UICorner", EditBtn).CornerRadius = UDim.new(0, 6)

EditBtn.MouseButton1Click:Connect(function()
    IS_GLOBAL_LOCKED = not IS_GLOBAL_LOCKED
    if not IS_GLOBAL_LOCKED then
        EditBtn.Text = "EDITING"; DimBackground.Visible = true; SizeMenu.Visible = true
        tweenObj(DimBackground, {BackgroundTransparency = 0.5}, 0.3); tweenObj(BlurEffect, {Size = 15}, 0.3)
    else
...
createFeature("Speed Glitch", btnGlitch, 1, {
    {text = "Speed", min = 150, max = 24000, default = 800, step = 10, var = "targetSpeed", type = "main"},
    {text = "Speed Time (s)", min = 0.1, max = 10, default = 3, step = 0.1, var = "accelTime", type = "main"},
    {text = "Cam Spin Angle (°)", min = 10, max = 360, default = 270, step = 5, var = "camSpinSpeed", type = "main"},
    {text = "Cam Spin Delay (ms)", min = 1, max = 100, default = 15, step = 1, var = "spinWaitMs", type = "main"},
    {text = "Offset Glitch (Studs)", min = -50, max = 50, default = -25, step = 1, var = "offsetGlitch", type = "legit"}
}, {
    {
        text = "Speed Glitch (Legit)",
        callback = function(state)
            isLegitSpeedEnabled = state
            btnGlitch.Visible = (isMainSpeedGlitchEnabled and not state)
            if not state then btnMacro.Visible = false end
            if state and isMacroEnabled and isMainSpeedGlitchEnabled then btnMacro.Visible = true end
        end
    },
    {
        ...
        callback = function(state)
            ...
            btnMacro.Visible = (state and isLegitSpeedEnabled and isMainSpeedGlitchEnabled)
        end
    }
})
...
createFeature("Noclip Push (Ultra)", btnUltra, 4, {
    {text = "Push Power", min = 1, max = 300, default = 180, step = 1, var = "noclipPower", type = "main"},
    {text = "Push Time (s)", min = 0.01, max = 1, default = 0.5, step = 0.01, var = "noclipTime", type = "main"}
})
...
OpenMenuBtn.Size = UDim2.new(0, 42, 0, 42); OpenMenuBtn.Position = UDim2.new(0, 20, 0.15, 0); OpenMenuBtn.Text = "S"; OpenMenuBtn.BackgroundColor3 = Color3.fromRGB(0, 0, 0); OpenMenuBtn.BackgroundTransparency = 0.2; OpenMenuBtn.TextColor3 = Color3.fromRGB(255, 255, 255); OpenMenuBtn.Font = Enum.Font.GothamBlack; OpenMenuBtn.TextSize = 18; OpenMenuBtn.Parent = ScreenGui; Instance.new("UICorner", OpenMenuBtn).CornerRadius = UDim.new(0, 10)
...
RunService.RenderStepped:Connect(function()
    if isMainSpeedGlitchEnabled and isLegitSpeedEnabled and player.Character then
...
RunService.Heartbeat:Connect(function()
    ...
    if states.glitch and not isLegitSpeedEnabled and player.Character then
        ...
        if root and hum and hum.MoveDirection.Magnitude > 0 then
            ...
            local currentRunSpeed = (elapsed / accelTime) * targetSpeed
...
btnUltra.MouseButton1Click:Connect(function()
    if not IS_GLOBAL_LOCKED or isNoclipping or not player.Character then return end
    isNoclipping = true
    local startTick = tick()
    local connection
    connection = RunService.Stepped:Connect(function(_, dt)
        if tick() - startTick >= noclipTime then connection:Disconnect(); isNoclipping = false; return end
        for _, p in pairs(player.Character:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide = false end end
        local hrp = player.Character:FindFirstChild("HumanoidRootPart")
        if hrp then hrp.CFrame *= CFrame.new(0, 0, noclipPower * dt) end
    end)
end)
